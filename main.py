...
def create_data_interaction_window(root, title="Cửa sổ tương tác dữ liệu"):
    ...
    def get_batch_diff_content(folder_path, file_names):
        ...
        # Dictionary dùng để nhớ xem [different keyword] của mỗi file tương ứng với chữ gốc nào
        ...
        for i in range(min_lines):
            ...
            for j in range(min_words):
                ...
                if all(w_list[j] == first_word for w_list in words_list):
                    # GIỐNG NHAU
                    if in_diff:
                        # Chốt lại phần khác biệt trước đó và lưu vào mapping
                    ...
                    if not in_diff:
                        ...
                        in_diff = True
                    # Lưu nháp lại các từ khác nhau của từng file
                    for idx in range(len(file_names)):
                        current_diff_chunks[idx].append(words_list[idx][j])
        
            # Xử lý phần từ khóa bị dư ra ở cuối dòng (do độ dài khác nhau)
            ...
            # Nếu kết thúc dòng mà vẫn đang trong trạng thái khác biệt thì chốt map lại
            if in_diff:
                for idx, fname in enumerate(file_names):
                    mapping[fname].append(" ".join(current_diff_chunks[idx]))
        ...
        # Xử lý các dòng thừa ra nếu file có số lượng dòng khác nhau
    ...
    # =====================================================
    # GIAO DIỆN UPDATE (NÂNG CẤP CHỌN FILE BẰNG MÀU SẮC & NÚT SỬA AN TOÀN)
    # =====================================================
    def open_update_window():
        ...
        # Biến lưu trữ Mapping cho chế độ sửa hàng loạt
        batch_mapping_data = {}
        ...
        def apply_edit():
            ...
            # --- Xử lý chế độ Hàng loạt (Có mapping) ---
            elif mode == "Hàng loạt":
                ...
                try:
                    for filename in active_files:
                        # Lấy danh sách các từ gốc của file này từ bản đồ lưu trữ
                        chunks = batch_mapping_data.get(filename, [])
                    
                        # Tách template ra dựa trên chữ [different keyword]
                        parts = new_content.split("[different keyword]")
                    
                        # Cảnh báo bảo mật logic: Nếu người dùng lỡ tay xóa mất [different keyword]
                        if len(parts) - 1 != len(chunks):
                            messagebox.showerror("Lỗi Cấu Trúc", f"Bạn đã vô tình xóa hoặc thêm cụm từ '[different keyword]' trong lúc chỉnh sửa.\n\nHệ thống yêu cầu phải giữ nguyên số lượng các từ khóa này để có thể ráp đúng dữ liệu cũ vào từng file!\n\n(Không thể lưu file)", parent=upd_window)
                            return
                        
                        # Lắp ráp lại văn bản: Các đoạn giống nhau + dữ liệu khác nhau gốc của file
                        reconstructed_text = ""
                    ...
                    messagebox.showinfo("Thành công", f"Đã áp dụng thành công lên {success_count} file!\n\nDữ liệu chung đã được cập nhật, dữ liệu riêng biệt của từng file được bảo tồn.", parent=upd_window)
                except Exception as e:
        ...
        # --- HÀM CẬP NHẬT VIEW (Nâng cấp thêm gán Mapping) ---
        def update_batch_view():
            ...
            if not active_files:
                ...
                # Nhận về văn bản chung và Từ điển mapping
                ...
                # Khung text vẫn mở khóa để người dùng sửa
